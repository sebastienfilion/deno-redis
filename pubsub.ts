...
class RedisSubscriptionImpl implements RedisSubscription {
  ...
  async psubscribe(...patterns: string[]) {
    await sendCommand(
      this.connection.writer! as BufWriter,
      this.connection.reader! as BufReader,
      "PSUBSCRIBE",
      ...patterns
    );
  ...
  async *receive(): AsyncIterableIterator<RedisPubSubMessage> {
    ...
    while (this.isConnected) {
      try {
        const rep = (await readArrayReply(this.connection.reader as BufReader)) as string[];
        const ev = rep[0];
        ...
        if (error.message === "Invalid state" || error instanceof Deno.errors.BadResource) forceReconnect = true;
        else throw error;
      } finally {
        if ((!this.isClosed && !this.isConnected) || forceReconnect) {
          ...
          if (Object.keys(this.channels).length > 0) await this.subscribe(...Object.keys(this.channels));
          if (Object.keys(this.patterns).length > 0) await this.psubscribe(...Object.keys(this.patterns));
        }
      }
    }
  }
